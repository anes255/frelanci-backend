...
exports.approvePayment = async (req, res) => {
  try {
    ...
    if (order.freelancerId._id.toString() !== req.user.userId) {
    ...
    const systemMessage = {
      senderId: order.freelancerId._id,
      senderName: 'System',
      message: `💰 Payment of ${order.price} DA has been confirmed by ${order.freelancerId.name}`,
      isSystemMessage: true,
      createdAt: new Date()
    };
    
    order.messages.push(systemMessage);
    await order.save();
